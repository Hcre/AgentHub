...
# 注：v3「前门机械反射」(ChatService._is_control 关键词命中即取消) 已在 v4 移除——
# 取消/停止判定改由 ReactiveRouter 的 LLM route_message 工具承担（含「停一下/等一下
# 不算 cancel，应 relay 讨论」的语义细分），机械关键词命中不再适用。原
# test_reflex_control_hits/misses 测的是已删行为，随之移除。
