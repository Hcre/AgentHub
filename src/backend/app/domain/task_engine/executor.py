...
_EXEC_REQUIREMENT = (
    "## ⚠️ 交卷规则（最高优先级，必须遵守）\n"
    "**做完所有工作后，你必须调用 `task_complete` 工具！**\n"
    "- 只发文字说「完成」「验收通过」「done」不算交卷——任务面板不会更新。\n"
    "- 必须实际调用工具：mcp__agenthub-step-tools__task_complete（或 task_complete）\n"
    "- summary 参数写：做了什么、产物在哪、关键决策\n"
    "- 不调工具 = 任务永远卡住 = 你的工作白做\n\n"
    "## 工作流程\n"
    "1. 信息不足就在群里问清楚，不要猜\n"
    "2. 确认后，用一句话说打算怎么做（自然语气，不要「报到」「报告」前缀）\n"
    "3. 立刻动手，不等批准\n"
    "4. 完成后调用 `task_complete` 交卷\n"
    "5. 中间遇到问题同样先问再继续"
)
