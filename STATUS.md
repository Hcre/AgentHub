...
> 更新:**2026-06-10 v2-status 后端镜像重建 + CLI root 修复 (袁, 同分支; ①重建 agenthub-celery_worker 镜像 → M4 路由+后台 session 修复随镜像永久生效, 重建后 deploy 闭环复测 POST→ready→/preview/{id}/index.html 200+真HTML; ②Dockerfile 加 IS_SANDBOX=1 → 容器内 claude --dangerously-skip-permissions 由「root/sudo 拒绝」转「Not logged in」, 已过 root 闸; 余鉴权需配 API key 属凭证范畴本环境无; 12 例历史后端失败已全修, 后端套 367 passed/0 failed; M1#3 落地页发送空会话兜底已修; commit d704071 等; 本地未推)** +
> 更新:**2026-06-10 v2-status 5 步真实验证 (袁, 分支 feature/v2-status/m1-m3-m4-impl; 执行报告 §9 五步: ①DB 0024 已 head + archived 列/索引在库 ②新增 6 套单测全绿 test_session_archived/test_static_host/test_fs_endpoints_v2/test_agent_draft + 修 4 例被 M4 异步改造打破的 deploy 回归 ③vite(3000)+后端(8200) 真浏览器: M1#1 三栏+36会话回灌+无白屏截图; M1#3 消息 E2E 阻塞(离线 agent 发送 disabled, 不补假图) ④M4 闭环 live 验过 POST→ready→/preview/{id}/index.html 200+真HTML + package 真zip ⑤本地 commit, 等 user 说推; **过程查出并修 2 个真 bug: main.py 从未 include deploy.router(全 /api/deployments 404) + deploy_service 后台 task 复用请求 session 致永久卡 queued**; 另 12 例历史既存失败与本期无关; per [[feedback-no-fake-evidence]])** +
