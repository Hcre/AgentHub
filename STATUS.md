...
> **最近更新: 2026-06-10 17:30**（袁, 分支 main）。完整变更历史见下方「更新日志」。
> **本会话 14 commit 已 rebase 到 origin/main（含队友 8 新提交）之上并 push（`6e1095c..bda3e4e`）**；功能完整度 76%→**91%**（绿测真相 + 私聊死路 + PPT + 版本历史 + 对话式代码修改 + FSM/DAG 派发 + 对话式创建 Agent + **部署发布真实流水线**）。下方各条目里 "未 push / 待 push" 均已作废（已全部 push）。
...
- **2026-06-10 17:30 — 部署发布真实流水线落地（⚠️→✅）+ 4 陈旧测试对齐** (袁, 分支 main, 已 push)：PRD §5 部署「预览 URL / 静态站点 / 源码打包」由"假 URL 桩"升级为**真实可访问托管**。**根因 bug**：`deploy.router` **从未在 `main.py` 注册**（前端 `api/deploy.ts`/`DeployPanel` 全在，但后端端点 404 死路）—— 已补 `app.include_router(deploy.router)`。**后端**：新增 `infrastructure/deploy/static_host.py`（files 落盘 `_assets/deploy/{id}/` + 路径穿越防护 + zip 打包）+ `main.py` 挂 `StaticFiles` 于 `/preview` + `DeployService._advance_synchronous` 真实落盘（static_site→`{public_base_url}/preview/{id}/{entry}`，package→真 `site.zip` 下载，container→诚实 `E_DEPLOY_CONTAINER_UNSUPPORTED` 不发假 URL）+ `delete` 清盘 + 配置 `deploy_root`/`public_base_url`。**前端**：`DeployPanel` 加「新建部署」模态（HTML 编辑→`deployApi.start`，全仓**首个** start 调用方，闭合"前端零触发"缺口）。**测试**：后端 deploy 19 pytest 重写为真实落盘断言（含路径穿越/container 失败/清盘）+ 前端 3 vitest（start 参数 + reload）。**live E2E（真浏览器，0 deploy 报错）**：① curl POST static_site→ready→`GET preview_url` 返 200 服务真 HTML；② package→真 212B zip 下载 200；③ container→failed 诚实；④ Playwright 全链路 新建预览→部署 tab→新建→部署→「已就绪」行 + preview 链接，截图 `deploy-served-site-live` / `deploy-panel-create-form` / `deploy-panel-ready-row`。**附带**：对齐 4 个陈旧测试（`test_claude_code_runtime` 2 个 `_build_env` 旧 env 注入断言、`test_mcp_opencode_inject` 2 个 `_write_opencode_config` 旧签名/provider 注入），实测**后端 375 passed/3 skipped、前端 143 passed**、ruff/mypy（本次文件 0 err）/tsc/eslint 绿。功能完整度 89%→**91%**
...
| | 预览 URL / 静态站点 / 容器化 / 源码打包 | ✅ 完整 | **6/10 袁 真实流水线**：补注册 `deploy.router`（曾漏挂 main.py→404）+ `StaticHost` 落盘 + `/preview` StaticFiles 托管 + `DeployPanel` 新建部署触发；live：static_site→served HTML 200 / package→真 zip 200 / container→诚实 failed；Playwright 全链路 + 3 截图。容器化诚实标记未支持（无宿主 Docker），非假 URL | 袁 |
...
**整体覆盖率**: ✅ 完整 24 / ⚠️ 部分 1 / ❌ 未做 1 / 📋 计划 1 = 共 27 项 → 覆盖率 = (24 + 1×0.5) / 27 = **91%**（6/10 17:30 部署发布 ⚠️→✅ 真实流水线 by 袁；唯一剩 ⚠️ = 消息操作真 URL E2E（功能已全实现），❌ = 代码冲突处理，📋 = 桌面端）
...
| **11** | **Memory update 接口** (`memories.py`路由在 `/api/agents/{id}/memories`) `PATCH /api/agents/{id}/memories/{mid}` | ✅ **已落地**（6/10 袁复核 — 此前"待复核"系失真：grep 只查了 MemoryPanel，实际调用在 store）：`MemoryPanel` 铅笔按钮→`EditForm`→`memoryStore.updateMemory`→`memoriesApi.update`(PATCH) 全链路在；togglePin 同走 PATCH。补 1 vitest 回归锁定（编辑内容→保存→断言 PATCH 入参） | PRD §3 长期记忆编辑路径已通 | ✅ DONE |
