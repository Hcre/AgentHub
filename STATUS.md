...
- **MCP P2 核心已落地**（分支 `feature/mcp/p2-binding-attach`）：
  ...
  - ✅ **收束-2（F2）闭合（2026-06-04）**：四阶段双线签核（[收束报告-F2](docs/reports/收束报告-MCP-F2.md)）；MCP 专项 34/34 绿，AI 线无红线。
...
| ~~MCP 注入 claude_code-only（R11）~~ ✅ opencode 已拉回（ADR-06，`OPENCODE_CONFIG` 逐进程通道+8 测试）；**pi_agent 仍 deferred**（本机无 pi 二进制可验证）| P2 运行时审计 | 🟢 低 | pi_agent 待上游 MCP 支持（解除门 RT-MCP §3.3）|
