...
| **AR-02** | **新 Agent 系统只加 Adapter**：禁止改 `backend/app/domain/` 适配新系统 | `import-linter` forbidden 契约 + CR 审查 |
| **AR-03** | **Harness 不含 LLM 调用**：`backend/app/domain/task_engine.py` 全部确定性 Python，LLM 输出为结构化 JSON 由 Harness 校验执行 | grep `httpx/openai/anthropic` in `domain/` |
...
**Python — `backend/pyproject.toml` 追加：**
...
**TypeScript — `frontend/.dependency-cruiser.cjs`**：配 `no-circular` 与 UI/store/api 层间禁向规则，挂入 pre-commit。
...
| 这段代码是…… | 放哪层 | 对应目录 | 不应包含 |
|--------------|--------|----------|----------|
| React 组件、页面、UI 状态 | L5 | `frontend/src/` | 业务规则 |
| HTTP 路由、WS endpoint、Pydantic 入参 | L4 | `backend/app/api/` | 业务逻辑、DB 访问 |
| Service 用例编排、事务边界、Command/DTO | L3 | `backend/app/application/` | 实体规则、外部调用细节 |
| 实体（Agent/Task/Group/Message）、Task Engine、领域服务、Repository 接口 | L2 | `backend/app/domain/` | DB / Redis / HTTP 框架（禁 import） |
| LLM Adapter、Repository 实现、Redis、WS 实现、Coordinator | L1 | `backend/app/infrastructure/` | 业务判断 |
| Pydantic v2 IO 模型 | 跨层（DTO） | `backend/app/schemas/` | 业务规则 |
| DI 容器、配置加载、CORS / 日志中间件 | 横切 | `backend/app/core/` | 业务规则 |
...
| 维度 | SDK 模式 | CLI 模式（默认） |
|------|---------|-----------------|
| 实现位置 | `backend/app/infrastructure/llm/sdk/` | `backend/app/infrastructure/llm/cli/` |
...
1. 在 `backend/app/infrastructure/adapters/` 创建 `xxx_adapter.py`
2. 继承 `base.LLMAdapter`，实现 `chat()` / `stream_chat()` / `get_capabilities()`
3. 内部通过 `provider + model + api_key + base_url` 配置底层 LLM
4. **禁止**修改 `backend/app/domain/` 中任何代码以适配新系统

适配器接口契约见 [docs/specs/04c-adapter-interface_适配器接口规范.md](../docs/specs/04c-adapter-interface_适配器接口规范.md)。

### 4.3 Task Engine 与 Harness（落实 AR-03 / AR-05）

- **Harness** = `backend/app/domain/task_engine.py`：纯 Python 确定性逻辑（FSM 转换、Guard、幂等校验、环检测、预算校验）
...
```python
# backend/app/api/chat.py（L4 表现层）
@router.post("/chat")
...
```python
# backend/app/domain/task_engine.py
...
| 验收标准 | [docs/specs/01-architecture_架构定义.md](../docs/specs/01-architecture_架构定义.md) |
| 数据流细节 | [docs/specs/01b-architecture-design_分层与数据流.md](../docs/specs/01b-architecture-design_分层与数据流.md) |
| 适配器接口 | [docs/specs/04c-adapter-interface_适配器接口规范.md](../docs/specs/04c-adapter-interface_适配器接口规范.md) |
| CLI 流程 | [docs/specs/04b-adapter-cli-flow_适配器CLI流程分析.md](../docs/specs/04b-adapter-cli-flow_适配器CLI流程分析.md) |
