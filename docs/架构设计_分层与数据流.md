...
│  │                                                                      │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────────┐      │ │
│  │  │PG Repo   │ │Redis Cache│ │Celery    │ │ 双轨适配 (v2.1)  │      │ │
│  │  │SQLAlchemy│ │· Session │ │ Workers  │ │                   │      │ │
│  │  │· CRUD   │ │· RateLim │ │· Canvas  │ │ LLMAdapter (API)  │      │ │
│  │  │· Migrate│ │· Pub/Sub │ │· Retry   │ │ ├ClaudeAdapter    │      │ │
│  │  └──────────┘ └──────────┘ └──────────┘ │ ├OpenAICompat    │      │ │
│  │                                         │ └MockAdapter     │      │ │
│  │  ┌──────────┐ ┌──────────┐              │                   │      │ │
...
| **L1 Infrastructure** | 持久化、缓存、消息队列、LLM 调用、文件系统 | PG 表结构、Redis 命令、Celery 配置、LLM SDK / CLI 子进程 | 业务逻辑、UI、路由 | 实现 L2 定义的 Repository/Adapter 接口（双轨：LLMAdapter + AgentRuntime） |
