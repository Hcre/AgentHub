...
> 版本：v1.0 | 日期：2026-05-21 | 基于 PRD v1.0
...
│  │  │  │  + Agent Registry + Shared Artifacts  │          │          │ │
│  │  │  └───────────────────────────────────────┘          │          │ │
│  │  └──────────────────────────────────────────────────────┘          │ │
│  │                                                                      │ │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐                │ │
...
│  │                                                                      │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────────┐      │ │
│  │  │PG Repo   │ │Redis Cache│ │Celery    │ │ LLM Adapter     │      │ │
│  │  │SQLAlchemy│ │· Session │ │ Workers  │ │ ┌─────────────┐ │      │ │
│  │  │· CRUD   │ │· RateLim │ │· Canvas  │ │ │ClaudeAdapter│ │      │ │
│  │  │· Migrate│ │· Pub/Sub │ │· Retry   │ │ │GPTAdapter   │ │      │ │
│  │  └──────────┘ └──────────┘ └──────────┘ │ │CodexAdapter │ │      │ │
│  │                                         │ └─────────────┘ │      │ │
│  │  ┌──────────┐ ┌──────────┐              └──────────────────┘      │ │
│  │  │FileSystem│ │WS Bridge │                                        │ │
│  │  │· Sandbox │ │· Redis→WS│                                        │ │
│  │  │· Virtual │ │· Broadcast│                                       │ │
│  │  └──────────┘ └──────────┘                                        │ │
│  └────────────────────────────────────────────────────────────────────┘ │
...
| **L1 Infrastructure** | 持久化、缓存、消息队列、LLM 调用、文件系统 | PG 表结构、Redis 命令、Celery 配置、LLM SDK | 业务逻辑、UI、路由 | 实现 L2 定义的 Repository/Adapter 接口 |
...
- L2 **不依赖** L1，L2 定义 `AgentRepository` / `TaskRepository` / `LLMAdapter` 等抽象接口
- L1 **实现** L2 定义的接口
