...
>
> AgentHub 接口完整定义在 [docs/specs/04-commands_命令接口.md](../specs/04-commands_命令接口.md)；本规范定义**设计原则**而非端点清单。AgentHub 用 FastAPI（async + Pydantic v2），主路径走 WebSocket，HTTP 用作配置/创建/历史查询。
...
**FastAPI 全局异常处理（`src/backend/app/api/error_handlers.py`）**：
...
**Pydantic v2 输入模型（`src/backend/app/schemas/agent.py`）**：
...
**WS 消息契约（`src/backend/app/api/ws/protocol.py`）**：
...
| 完整 endpoint 清单 | [docs/specs/04-commands_命令接口.md](../specs/04-commands_命令接口.md) |
