...
> 基准：`PRD_AgentHub_v4_统一方案.md` + `架构设计_分层与数据流.md` + **已交付脚手架**。
> 时间轴：比赛 20 天里程碑 M1–M6（5/20–6/10）。
> 核心原则：**每人从数据库到前端完整交付一个功能域，可独立开发、测试、演示。**
>
> ⚠️ v4 PRD 架构变更影响：
> - Celery Canvas → `asyncio.gather`（任务 2.8/2.10 受影响）
> - LiteLLM 网关 → 移除，改为双轨 SDK/CLI（任务 2.11 受影响）
> - task_events/task_artifacts/notifications 表 → 砍掉（任务 2.7/3.8 受影响）
> - 12 表 → 6 表
>
> 上方任务描述保留 v3 原始规划，实际实现以 v4 PRD 和当前代码为准。
...
| v2 问题 | PRD 依据 | v3 修正 |
|---------|---------|---------|
| Agent 管理压成 "Registry 4h" | §2.1 详情页 6 Tab + 对话式创建 + 权限矩阵 | 升级为董 核心模块（≈26h） |
| 收件箱整块缺失 | §2.5 审批流四决策 + 通知 + 日历 | 黎 独立模块（≈22h） |
| 任务管理界面缺失 | §2.4 看板/筛选/父任务详情 | 黎 独立模块（≈18h） |
| AI 协作沉淀未分工 | §七 占评审 **30%** | 第八章显式分工，DRI = 董 |
...
```
用户视角                          → 域             → 架构落点
"选 Agent 聊天，消息能发能收能存"  → 袁 会话交互    → ChatService/WS/L1注入 (S8–S12,S22–S24)
"建 Agent、建群、丢需求自动分解"   → 董 Agent与编排 → Agent/Group/Coordinator/TaskEngine (S1–S7,S13–S16)
"管任务、批审批、看Diff/预览产物"  → 黎 管理与产物  → Inbox/TaskBoard/Artifact/记忆 (S17–S21,产物内联)
```

### 袁：会话与交互层（IM Core）
**闭环**：消息能发能收能存能查，单聊+群聊+流式+多会话+三层上下文。
- 后端：`ChatService`、`SessionService`、WebSocket、MentionRouter（@解析路由）、L1 上下文注入消费、长对话压缩。
- 前端：主界面三栏布局 + 导航、`ChatView`/`SessionList`/`MessageList`/`StreamingText`、@补全菜单、**产物卡片渲染容器**（容器框架由袁，具体卡片组件由黎提供）。
...
### 黎：管理界面·收件箱·产物工具链（Surfaces & Tools）
...
|1.3|M2|✅ 聊天 UI 完善：三栏布局 + SessionList 排序/置顶/搜索 + MessageBubble 角色样式|8h|1.1|§4.1,§2.2.4|> 完成: 2026-05-22, 备注: SessionList+MessageBubble+Sidebar集成；三栏第三栏待董 Agent详情页|
|1.4|M2|🔄 流式细节：thinking 事件展示 + 错误重连 + token 计量条|5h|1.1|S12|> 完成: 2026-05-22, 备注: thinking事件+WS重连已完成；token计量条待后端暴露token count
|1.5|M3|@mentions：解析 + 前端自动补全菜单 + 路由（@Agent/@All/@协调者）|6h|董 群组|S9,S10,§2.2.3|
|1.6|M3|群聊消息广播：Redis Pub/Sub + 多客户端同步 + 协调者卡片占位|6h|1.5,董|S8|
|1.7|M3|多会话并行：SessionManager + 上下文隔离 + 会话归档|5h|1.3|§2.2.4|
|1.8|M4|三层上下文：热窗口(黎 L1)+Pin长期+历史摘要占位 + 长对话压缩触发|6h|1.2,黎 L2|S22,S24,§2.2.5|
|1.9|M4|产物卡片渲染容器：按 content_type 分发渲染（黎 提供卡片组件）|5h|黎 卡片|§2.2.6|
...
|2.12|M4|权限审批矩阵：危险操作(删除/push/deploy/外网)→AWAITING_APPROVAL|5h|2.7,黎 收件箱|§2.1.4,S18|

**小计 ≈78h**（最重，编排是系统大脑）→ 接受袁/黎 支援。

### 黎 — 管理界面·收件箱·产物工具链

| # | 里程碑 | 任务 | 工时 | 依赖 | PRD/场景 |
|---|--------|------|------|------|----------|
|3.1|M1|L1 记忆完善：滑动窗口 + 上下文注入接口（供袁/董）|3h|脚手架|MVP6,S22|
|3.2|M2|任务管理后端：`TaskService` 查询/多条件筛选/排序/分页|6h|董 tasks 表|S17,§2.4.2|
|3.3|M2|TaskBoard 前端：列表 + 看板(按状态分列) + 筛选栏 + 手动创建|8h|3.2|§2.4.2,§2.4.3,S16|
|3.4|M3|父任务详情页：子任务列表 + DAG 状态 + 产物汇总|5h|3.2,董 DAG|§2.4.4|
|3.5|M3|收件箱后端：通知生成 + 未读计数(Redis) + 分类查询|6h|董 事件|S20|
|3.6|M3|审批流：APPROVE/REJECT/EDIT/RESPOND 四决策 + checkpoint 恢复|6h|3.5,董 HITL|S18,S19,§2.5.2|
...
|3.10|M4|L2 摘要记忆：超阈触发 + 分层摘要 + PG 存储（供袁 上下文）|5h|3.1|§2.2.5,S24|
...
| 成员 | 域 | 自有 | +支援 | −被支援 | 实效 |
|------|----|------|-------|---------|------|
| 袁 | 会话交互 | 57h | +20h(支援董) | — | **77h** |
| 董 | Agent编排 | 78h | +12h(黎支援) | −20h | **70h** |
| 黎 | 管理产物 | 68h | — | −12h | **56h**＋打磨/E2E ≈20h = **76h** |

趋于均衡（≈76h/人，20 天约 3.8h/天）。支援安排：

| 阶段 | 支援方 → 被支援方 | 内容 |
|------|------------------|------|
| M2 | 袁 → 董 | Agent 详情页/对话式创建前端组件 |
| M3 | 袁 → 董 | Coordinator 联调 + TaskPlanCard 前端 |
| M3 | 董 → 黎 | 收件箱事件桥接 + HITL checkpoint 协议 |
| M5 | 黎 → 全员 | E2E + Demo 视频统筹 |
...
- 袁 发 `SendMessageCommand` → 产 `MessageSent`/`Streaming*` 事件。
- 董 `CoordinatorService.decompose_and_dispatch()` → 产 `TaskCreated`/`SubTaskCreated`/`TaskStateChanged`。
- 董 危险操作 → `ApprovalRequested`；黎 消费生成 `Notification` → `ApprovalResolved` 回传董 恢复执行。

### 6.2 适配器契约（已冻结，见 `adapter_interface_spec.md` / `domain/llm/protocol.py`）
- 董 实现 `UnifiedAgent`，产出 `StreamEvent`；袁 只依赖抽象消费流。
- Tool 调用：董 `ToolScheduler` → 黎 `ToolRegistry.register(DiffTool/DeployTool/...)`。

### 6.3 数据表归属

| 表 | 属域 | 跨域权限 |
|----|------|---------|
| `sessions`,`messages` | 袁 | 董 可读（Agent 上下文） |
| `agents`,`groups`,`group_members`,`tasks`,`task_events` | 董 | 袁/黎 可读 |
| `notifications`,`task_artifacts`,`knowledge_vectors` | 黎 | 董 可写产物 |

### 6.4 WebSocket 通道
| 通道 | 发布 | 订阅 |
|------|------|------|
| `session:{id}:message` | 袁 | 董 |
| `session:{id}:task_event` | 董 | 袁（聊天卡片） |
| `session:{id}:artifact` | 董 | 黎（触发 Diff/预览） |
| `inbox:{user}:update` | 黎 | 袁（未读 Badge） |
...
M1(脚手架已就绪) → M2 单聊MVP        → M3 群聊+协调者         → M4 产物+自建Agent → M5 打磨/文档/视频 → M6 提交
                  袁:私聊UI/CRUD     董:Coordinator/FSM/DAG  黎:Diff/预览/收件箱   全员联调            提交
                  董:Adapter实装     袁:@/群聊广播           董:权限/Codex        黎:E2E+Demo
                  黎:TaskBoard       黎:任务详情/审批后端     袁:产物容器
```

- **关键路径**：董 Coordinator(2.6)→TaskEngine(2.7)→DAG(2.8) 是 M3 成败核心；袁 群聊(1.6)、黎 任务详情(3.4)/审批(3.6) 均依赖它 → 董 M3 优先级最高，提前并接受支援。
- **降级闸**：Coordinator 不稳 → 降级手动 @Agent（PRD §九）；Adapter 阻塞 → mock 保流程。

---

## 八、AI 协作能力沉淀（评审 30%，硬指标，DRI=董）
...
| 成员 | 必交 | 验收 |
|------|------|------|
| 董 | Claude Adapter 真实流式 + Agent CRUD/详情页 | `/api/agents` 全链路 + 真实模型流式回复 |
| 袁 | 私聊 UI + 会话历史 + 流式打磨 | 浏览器内完成"需求→代码→流式展示"闭环 |
| 黎 | TaskBoard（列表+看板+筛选）+ 手动建任务 | 任务可创建可筛选可看板展示 |
