...
### 4.4 当前 session_id 存储与流转（现状分析）

#### 创建链路

```
POST /api/sessions {type, group_id?, agent_id?}
  → SessionService.create()
    → Session(id=uuid4(), type, group_id, agent_id)   ← Python uuid4() 生成
      → PostgresSessionRepository.save()
        → INSERT INTO sessions (id, type, title, ...)  ← PG 持久化
```

#### session_id 在系统中的三次使用

```
① PG sessions.id                 主键，查 session 元信息
② Redis l1:{session_id}          L1 滑动窗口 key
③ CLI --session-id {session_id}  CLI 磁盘目录名
...
```python
# chat.py:56-62 — WS handler 直接用 URL 里的 session_id
cmd = SendMessageCommand(session_id=session_id, ...)

# chat_service.py:100-108 — 同一个 session_id 三用
window = await self._l1.get_window(cmd.session_id)       # → Redis: l1:{session_id}
request = AgentRequest(session_id=cmd.session_id, ...)    # → 传给 adapter

# claude_code_runtime.py — 直接复用 AgentHub session_id 作为 CLI key
session_key = str(request.session_id)
cmd = ["claude", "--session-id", session_key, ...]        # → ~/.claude/sessions/{key}/
```
...
```
AgentHub session_id ──直接──→ CLI --session-id
```

私聊没问题（一对一），群聊会冲突：

```
群聊 session "def-456" 里有 3 个 Agent:
  ChatService 调 FrontendAgent → CLI: --session-id def-456
  ChatService 调 BackendAgent  → CLI: --session-id def-456   ← 同一个文件！
  ChatService 调 ReviewerAgent → CLI: --session-id def-456   ← 互相覆盖
```

#### 解决方案：插入 SessionManager

```
AgentHub session_id ──→ SessionManager.cli_key() ──→ CLI --session-id
                              │
                  私聊: str(session_id)
                  群聊: f"{session_id}:{agent_id}"
```

SessionManager 当前代码中不存在，需要在 ChatService 和 ClaudeCodeRuntime 之间新增。
