...
> 日期：2026-06-08 | 状态：后端全落，前端待做

### 14.1 问题

v4 后端（R0→R5 + abort）已全落 main，但前端对任务执行过程几乎不可见：
- 组员收到任务后一声不吭直接干活（`executor.py:35` 契约明令禁止「不要闲聊」）
- `_emit_update(node, "running")` 只推 WS `task_update` 事件到 `_progress` 面板，不进 messages 表
- 前端 CoordinatorPlan 组件是静态 mock，不消费 `task_update`
- 用户看到的体验：任务发出 → 黑箱 → 突然完成

### 14.2 核心原则：状态进面板，声音进聊天

| 信息 | 放哪 | 为什么 |
|------|------|--------|
| 分配（谁领了什么） | 协调者 `_post_plan` 一条 | 一次说清，不用每人重复 |
| 执行状态（进行中/完成/阻塞） | 任务面板（step 变色，就地更新） | 面板是「我们到哪了」的真相，不占聊天滚动 |
| 组员的声音（收到、提问、交卷总结） | 聊天气泡 | 这是组员在「说话」，才是 IM 的核心 |

反面教材：一个 5 步并行 DAG，如果每个组员都发「收到」+「完成」，协调者再发计划+总结，就是 2N+2 条纯状态消息把真正的内容淹了。

### 14.3 「收到」怎么做：组员自己说（B），不用 Harness 模板（A）

| | A. Harness 发模板消息 | B. 组员自己说（推荐） |
|---|---|---|
| 谁说的 | SYSTEM 顶着组员名字 | 组员 LLM 真的在说话 |
| 实现 | orchestrator PENDING→RUNNING 时 `_post_message("阿强收到：实现后端，开工")` | 改 executor 契约，让 LLM 开工前先报到 |
| 成本 | 零 LLM | 零额外成本（本就是同一次 CLI 流，文本走 R1 已有通道进群聊） |
| 像人吗 | 不像。每条措辞一模一样，机器味 | 像。是真实的、带语气的应答 |
| 风险 | 无 | 可能 LLM 不照做 / 冷启动有几秒延迟 |

**选 B**，因为它正是 v4 §2.4 的字面意思——「worker 说话就是正常文本，推群聊」。不需要任何新协议、新消息类型。

### 14.4 落地清单

| # | 项目 | 位置 | 说明 |
|---|------|------|------|
| 1 | **改契约：组员开工先报到** | `executor.py:34` | 「不要闲聊」→「开工前先用一句话在群里说：你收到了什么任务、打算怎么做；然后立刻动手，不要停下来等批准」 |
| 2 | **验证回归** | 测试 | 跑多步任务，确认组员是「报到→干活」而非「报到→聊天等批准」 |
| 3 | **接活面板** | 前端 `CoordinatorPlan` | 消费 `task_update` WS 事件 → `groupStore` → live 组件，step 状态实时变色（pending→running→done/failed） |
| 4 | **里程碑降权** | `_post_step_done` | 聊天里的完成通知改细分隔行，不抢组员说话的视觉权重 |

**⚠️ 改契约的回归风险**：那句「不要闲聊」当初是故意写的——大概率是因为早期组员光聊天不干活。松开后措辞必须精确卡死边界：「一句话报到 + 立刻动手」，不能变成「可以聊天」。改完必须验证组员没有因此变得话痨/拖延。
...
① + ③ 一起做才完整：① 让「收到」出现在聊天里，③ 让「执行到哪了」在面板上可见。只做 ①，执行状况还是看不全——running、依赖、卡死这些结构化状态，聊天天生表达不好。
