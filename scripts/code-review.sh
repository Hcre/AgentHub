...
if cd src/backend && ruff check app/ --config pyproject.toml 2>/dev/null; then
    echo "✅ ruff 通过"
else
    echo "❌ ruff 失败"
    fail=1
...
echo ""
echo "► mypy 类型检查..."
if cd src/backend && mypy app/ 2>/dev/null; then
    echo "✅ mypy 通过"
else
    echo "❌ mypy 失败"
    fail=1
...
echo ""
echo "► tsc TypeScript..."
if cd src/frontend && npx tsc --noEmit 2>/dev/null; then
    echo "✅ tsc 通过"
else
    echo "❌ tsc 失败"
    fail=1
...
echo ""
echo "► eslint..."
if cd src/frontend && npx eslint src/ --config .eslintrc.json 2>/dev/null; then
    echo "✅ eslint 通过"
else
    echo "❌ eslint 失败"
    fail=1
...
cat << 'EOF'
架构红线 (docs/conventions/01-architecture):
...
代码红线 (docs/conventions/02-coding):
...
流程红线 (docs/conventions/99-process-rules):
