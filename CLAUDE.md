...
| "调用图 / 影响分析 / 代码地图" | 查 `docs/CODE_MAP.md`（后端模块全景 + 入口表）；AI 查 `.codegraph/graph.json`（节点/边/缺陷）；人看 `.understand-anything/graph.html`（浏览器交互，dashboard「图谱」Tab 内嵌）；重建跑 `python scripts/gen_codegraph.py`。规范见 `docs/conventions/08-code-understanding_*` |
