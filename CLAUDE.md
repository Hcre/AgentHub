...
IM 聊天式多 Agent 协作平台。5 层洋葱架构（L1 Infrastructure → L2 Domain → L3 Application → L4 API → L5 Presentation）。技术栈：FastAPI + React/TypeScript + PostgreSQL + Redis + Docker。LLM 接入：CLI（AgentRuntime`），CLI 优先。
