...
纯文档项目（Markdown 规范 + Python 示例），Git 管理。本身就是一套"可复制到新项目"的开发规范模板。34 个功能点全部交付，可投入使用。
...
| # | 规范 | 细化的流程环节 | 红线（详见各篇 §一）|
|---|------|---------------|------|
| 01 | 架构 | 第一步·设计 | 禁循环依赖 / 领域层不依赖框架 / 禁跨层调用 |
| 02 | 代码 | 第二步 §2.1 实现 + §2.3 审查 | 禁 print / SQL 参数化 / 密钥走环境变量 / 禁静默吞异常 / 日志脱敏 |
| 03 | Git | 第二步 §2.6 提交 | main 禁直推 / Conventional Commits / 禁 force push 共享分支 |
| 04 | API | 第二步 §2.1 接口 | 名词复数+kebab-case 无动词 / 统一响应 / 默认认证 / 向后兼容 |
| 05 | 测试 | §2.1 TDD + §2.2 验证 | 测试独立 / 只 Mock 外部边界 / 覆盖正常+边界+异常 |
| 06 | 文档 | §2.5 + 07-汇报 | 必有 README / 发布更 CHANGELOG / 文档随码同 PR |
| 08 | 图谱 | 第二步 §2.9 工具增强 | 仅中大型项目（>10 模块）启用 |
...
| 项 | 说明 |
|------|------|
| **汇报（AI 必做）** | 每功能点完成更新 `worklogs/YYYY-MM-DD_*.md` + `STATUS.md`（必做）；功能点级汇报**默认对话内联、不落盘**，只有收束报告落盘 `docs/reports/`（详见 `07-汇报.md`）|
| **收束节点（人触发）** | 计划时预设（默认每 3 功能点）写入 `开发清单.md` 头部成硬闸门。**到点未收束时 AI 拒绝开始下一功能点**并提醒；AI 不替人做收束 |
| **核心原则** | 不越界 / 不黑盒 / 不断档 / 不拖欠 / 不积压（见 `05-完整流程与核心原则.md`） |

---

## 当前状态

- **进度**：34/34 ✅（见 `STATUS.md`）
- **规范版本**：01-06/08 已升至 **v2.0**——每篇是 ai-workflow 对应步骤的细化（红线配自动检测 + 可复制落地配置 + 决策表）；调用图/图谱内容全部收拢进 08；`src/<维度>/` 配有对应的可运行落地配置
- **ADR**：6 个（`worklogs/decisions/0001-0006`）
- **收束节点**：尚无正式执行记录（v0.1 人审计待签核）
...
| "收束" | 由**人触发**执行四阶段；到达预设收束节点未收束时 **AI 主动拦截**。人确认后按 `06-第三步_收束节点.md` 执行 → ADR + 收束报告 |
